...
class ElectronDetector:
    def __init__(
        ...
        radius: float,
        energy: float,
        facing: Vector3D = np.array([-1, 0, 0]),
        updirection: Vector3D = np.array([0, 1, 0]),
        acceptance_angle_phi: float = np.pi * 2,
        acceptance_angle_theta: float = np.pi,
        number_of_samples_theta: int = 5,
        number_of_samples_phi: int = 5,
        max_number_of_steps: int = 10,
        ...
        if boundary_temperature is not None:
            ...
            self.boundary_temperature = (
                float(self.simulation.results.global_parameters["electronTemperature"]) * 11604.518
            )  # Converting from eV to K
        ...
        self.potential_name = potential_val[0][1]
        self.electric_field_from_potential = potential_val[0][0].mesh.compute_derivative(
            scalars=self.potential_name
        )
        ...
        self.e_field_mesh = utils.generate_efield_vector_property(
            self.simulation, self.electric_field_from_potential
    ...
    def backtrack(self):
        ...
        with Pool(
            processes=default_settings.Settings.number_of_threads, initializer=init_worker, initargs=(self,)
        ) as pool:
            updated_particles = list(
                pool.imap_unordered(
                    backtrack_worker,
                    range(len(self.particles)),
                    max(1, len(self.particles) // default_settings.Settings.number_of_threads),
                )
            )
    ...
    def calculate_count_spacecraft(self, electron: Electron):
        ...
        if electron.probability_photo is not None:
            electron.count_photo = (
                electron.probability_photo * photodensity / scipy.constants.elementary_charge
            )

        if electron.probability_secondary is not None:
            electron.count_secondary = (
                electron.probability_secondary * secdensity / scipy.constants.elementary_charge
            )
    ...
    def detect_if_sun_visible_photoelectrons(self, electron: Electron):
        ...
        direction_check = [
            sun_direction[i] * (electron.previous_position[i] - electron.position[i]) for i in range(3)
        ]
