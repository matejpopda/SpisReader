...
class ElectronDetector:
    def __init__(
        ...
        boundary_temperature: float = 103000,
    ) -> None:
        self.particles: list[Electron] = []
    ...
    def backtrack(self):
        ...
        with Pool(processes=MAX_PROCESSES, initializer=init_worker, initargs=(self,)) as pool:
            updated_particles = list(
                pool.imap_unordered(
                    backtrack_worker, range(len(self.particles)), max(1, len(self.particles) // MAX_PROCESSES)
                )
            )
