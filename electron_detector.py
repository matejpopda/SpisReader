...
class ScatterResultAccumulator(ResultAccumulator):
    ...
    def plot(self):
        ...
        data = {"X_position": xx, "Y_position": yy, "value": cc}
        df = pd.DataFrame.from_dict(data)
        df.to_csv("boundary.csv")

        data = {"X_position": hit_x, "Y_position": hit_y}
        df = pd.DataFrame.from_dict(data)
...
class ElectronDetector:
    ...
    def calculate_probability_spacecraft(self, electron: Electron):
        def calculate_temperature_from_plasma_potential():
            ...
            temperature = (
                (consts.elementary_charge / consts.k) * (density / gradient_density) * electric_field
            )
    ...
    def detect_colission_SC(self, electron: Electron):
        ...
        colided_faces_SC: list[Any] = spacecraft_mesh.find_cells_intersecting_line(
