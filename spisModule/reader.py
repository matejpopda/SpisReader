...
def get_simulation_preprocessing(path_to_preprocessing: Path) -> SimulationPreprocessing:
...
def get_simulation_results(path_to_results: Path) -> SimulationResults:
    default_instruments : list[DefaultInstrument] = get_default_instruments(path_to_results / "DefaultInstruments")
    ...
    global_parameters = dictionary_from_list_in_xml_node(ET.parse(path_to_results / "GlobalParameters" / "globalParameters.xml").getroot()) 
    # \Simulations\Run1\GlobalParameters

    numerical_kernel_output : NumericalResults = get_numerical_kernel_output(path_to_results / "NumKernel" / "Output", user_instruments) 
    ...
    extracted_data_fields:ExtractedDataFields = get_extracted_datafields(path_to_results / "OutputFolder" / "DataFieldExtracted")
    # \Simulations\Run1\OutputFolder\DataFieldExtracted

    return SimulationResults(default_instruments=default_instruments,
